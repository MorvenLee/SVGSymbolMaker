...
//第二层菜单
buttons=new Array();
buttons[0]=new	Array( "打开地图", "导入地图", "载入本地SVG地图", "另存为SVG", "保存用户地图","退出");
...
//
makeLink(document.getElementById('MenuL2G-2-0-0'), "http://127.0.0.1:8080/SVGGIS/ssesql.jsp");
//makeLink(document.getElementById('MenuL2G-2-3-0'), "http://192.168.1.155:8080/SVGGIS/ssesql.jsp");
//
makeLink(document.getElementById('MenuL2G-5-0-0'), "http://127.0.0.1:8080/SVGGIS/ssesql.jsp");
makeLink(document.getElementById('MenuL2G-5-1-0'), "http://127.0.0.1:8080/SVGGIS/ssesql.jsp");
makeLink(document.getElementById('MenuL2G-5-2-0'), "http://127.0.0.1:8080/SVGGIS/ssesql.jsp");
makeLink(document.getElementById('MenuL2G-5-3-0'), "http://127.0.0.1:8080/SVGGIS/ssesql.jsp");
makeLink(document.getElementById('MenuL2G-5-4-0'), "http://127.0.0.1:8080/SVGGIS/ssesql.jsp");
//
//---nancy add 20130403---	
...
function menuonclick(i,j,k){//whw
	...
	else if(menuno=="menu00"){//打开地图
		window.parent.switchmap();//程序没问题，白云区楼房、dengyufang可正常显示，其他不行，原因可能是上传的文件本身，背景色fill为white
	}
	else if(menuno=="menu02"){//载入svg地图
		window.parent.UploadLocalSVG();
	}	
	else if(menuno=="menu03"){//另存为svg
		window.parent.saveToLocal();
	}
	else if(menuno=="menu04"){//保存用户地图--需要用户先登录，才能保存
		window.parent.saveUserMap();
	}
