	...
	var destfileSrcArr = new Array() ;
 	...
 	{			
			...
			subXMLHTTP.open("post","../../dataProcess/doDataDecompress.jsp",true);
...
//判断是否是svgz类型文件
 function isSvgz(filename){
       //var filename= "aa.PNG3";
       if(filename==""){
          //alert("请上传图片!");
          return false;
       }else {
         if(!/\.(svgz)$/.test(filename)) {
       ...
       return true;
    	...
    	function handleDrop(evt){
    		...
    		var files = evt.dataTransfer.files;
				...
				if(files.length>1)
				{
					alert("一次只能上传一个svgz文件！");
					return true;
				}
				
				//alert(files[0].name);
				//检查文件类型
				if(!isSvgz(files[0].name))
				{
					alert("文件类型不正确，请上传svgz文件!");
					return true;
				}
				/*
	...
	BarText.appendChild(tv);	
    	...
    	function uploadFile(file, progressbar) {
				...
				xhr.open("POST", "../../dataProcess/DataDecompressUpload.jsp?fileName="+file.name);
