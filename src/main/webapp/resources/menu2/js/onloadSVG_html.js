	...
	var destfileSrcArr = new Array() ;
 	...
 	function transSubmit(fileUpSrc)
 	{			
 		//alert(fileUpSrc);
 		alert('正在上传，请稍后...');
		var listname = fileUpSrc.trim() ;
		uploadfinish(listname);
 	}
 ...
 function isSvg(filename){
	...
	BarText.appendChild(tv);	
    	...
    	function uploadFile(file, progressbar) {
				...
				xhr.open("POST", "../../dataProcess/onloadSVGUpload.jsp?fileName="+file.name);
...
function setFileHiddenSrc(xmlHttpReq,index)
{
	if(xmlHttpReq.readyState == 4)
	{
			...
			function uploadProgress(evt){				
				if (evt.lengthComputable) {
					...
					//alert(test);
						...
						setProgressVal(pBarID,percentage);
		...
		function setProgressVal(pBarID,percentage)
		{
			//var pBarID = evt.target.progressbar.id; 
			var pBar_BarTextID = pBarID + "_Bartext"; 
						
			var tmp1 = SvgMainDoc.getElementById(pBarID);
			var tmp2 = SvgMainDoc.getElementById(pBar_BarTextID);
			//alert(tmp1.getAttribute("width") + "---" +　tmp1.getAttribute("fill"));
			...
			tmp1 = SvgMainDoc.getElementById(pBarID);
			tmp2 = SvgMainDoc.getElementById(pBar_BarTextID);	
			//alert(tmp1.getAttribute("width") + "---" +　tmp1.getAttribute("fill"));
		}
