	...
	var destfileTypeArr = new Array() ;
 	...
 	function transSubmit(filepath,upLoadPath,attr_selVal,attryear,attrarea,mapname)
 	{
 		alert("开始导入数据库，请耐心等待。。。");			
			var sendStr = "filepath=" + filepath + "&upLoadPath=" + upLoadPath ; 
			sendStr = sendStr + "&attr=" + attr_selVal;
			...
			subXMLHTTP.open("post","../../dataProcess/ImportShape.jsp",true);
	...
	if(xmlHttp.readyState == 4)
	{
		if(xmlHttp.status == 200)
		{
      		...
      		//清空原信息
				...
				if(rt.trim()=="success")
				{
					SvgMainDoc.getElementById("disImportStatus").firstChild.nodeValue = "数据入库完成！";
				}
				else
				{
					SvgMainDoc.getElementById("disImportStatus").firstChild.nodeValue = "发生未知错误，数据入库失败！";
				}
...
//判断是否是shp类型文件
 ...
 //判断是否是shx类型文件
 function isShx(filename){
       //var filename= "aa.PNG3";
       if(filename==""){
         // alert("请上传图片!");
          return false;
       }else {
         if(!/\.(shx)$/.test(filename)) {
           //alert("图片类型必须是.gif,jpeg,jpg,png中的一种!")
           return false;
         }
       }  
...
function getUploadFileNamePre()
{
	...
	return datestr;
    	...
    	function handleDrop(evt){
    		...
    		var files = evt.dataTransfer.files;
				...
				if(files.length!=3)
				{
					alert("一次必须上传一个shp、一个shx和一个dbf文件！");
					return true;
				}
				...
				*/
				//检查文件类型
				/*
				if(!isShp(files[0].name) && !isShx(files[0].name) && !isDBF(files[0].name))
				{
						alert("文件类型不正确，请上传shp文件、或shx文件或dbf文件!");
						return true;
				}
				*/
				var shpStatus = false;
				...
				for (var i = 0; i < files.length; i++) 
				{
					if(isShp(files[i].name))
					{
						...
						shpStatus = true;
						destfileTypeArr.push("shp");
					}
					else if(isShx(files[i].name))
					{
						if(shxStatus == true)
						{
							alert("只能上传一个Shx文件!");
						...
						shxStatus = true;
						destfileTypeArr.push("shx");
					}
					else if(isDBF(files[i].name))
					{
						if(dbfStatus == true)
						{
							alert("只能上传一个DBF文件!");
						...
						dbfStatus = true;
						destfileTypeArr.push("dbf");
					}
					else
					{
						alert("文件类型不正确，请上传shp文件、或shx文件和dbf文件!");
						return true;
					}
				}
	...
	BarText.appendChild(tv);	
    	...
    	function uploadFile(file, progressbar,index,UploadFileNamePre) {
				...
				xhr.open("POST", "../../dataProcess/ImportShapeUpload.jsp?fileName="+file.name+"&savefilePre="+UploadFileNamePre+"&type="+filetype);
