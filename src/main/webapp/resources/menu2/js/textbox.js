...
function textbox(id,parentNode,defaultVal,maxChars,x,y,boxWidth,boxHeight,textYOffset,textStyles,boxStyles,cursorStyles,selBoxStyles,allowedChars,functionToCall) {
	...
	if (arguments.length == nrArguments) {
		...
		this.textboxursor = null; //later holds reference to cursor
...
textbox.prototype.createTextbox = function() {
	...
	if (result) {
		...
		this.textboxursor = document.createElementNS(svgNS,"line");
		this.textboxursor.setAttributeNS(null,"x1",this.x);
		this.textboxursor.setAttributeNS(null,"y1",(this.y + this.textYOffset + this.textStyles["font-size"] * 0.2));
		this.textboxursor.setAttributeNS(null,"x2",this.x);
		this.textboxursor.setAttributeNS(null,"y2",(this.y + this.textYOffset - this.textStyles["font-size"] * 0.9));
		for (var attrib in this.cursorStyles) {
			this.textboxursor.setAttributeNS(null,attrib,this.cursorStyles[attrib]);
		}
		this.textboxursor.setAttributeNS(null,"id",this.id+"Cursor");
		this.textboxursor.setAttributeNS(null,"visibility","hidden");
		this.textboxTextGroup.appendChild(this.textboxursor);
		...
		this.textboxTextContent.nodeValue="";
			...
			if (evt.currentTarget.nodeName == "g" || evt.currentTarget.nodeName == "svg:g") {
				...
				if (this.textboxStatus == 0) {
					...
					this.textboxursor.setAttributeNS(null,"visibility","visible");
					

			    	//make sure the functiontocall to be called after cursor appeared
					this.fireFunction();
				}
				else {
					evt.stopPropagation();
	...
	if (evt.type == "mousemove") {
		if (this.textboxStatus == 2 && this.shiftDown && this.mouseDown && this.supportsCharGeom) {
				...
				if (this.cursorPosition + 1 != this.startOrigSelection) {
					...
					var cursorX = parseInt(this.textboxursor.getAttributeNS(null,"x1"));
	...
	if (evt.type == "keypress") {			
		...
		if (keyCode > 31 && keyCode != 127 && keyCode < 65535 && evt.charCode && evt.charCode < 65535) {	
			...
			if (result == 0) {
				...
				if (textChanged) {
					//code textbox hides cypher by *
					if(this.id=="code")
					this.textboxTextContent.nodeValue +="*";
					else this.textboxTextContent.nodeValue=this.textVal;
					this.changed = true;
...
textbox.prototype.Delete = function(id,DELflag) {
if (DELflag == true) {

		//only do it if there is still text and cursor is not at start position
			if (id.textVal.length > 0 && id.cursorPosition > -2) {
				//first deal with text content, delete chars accor ding to cursor position
				if (id.shiftDown && id.selectionRectVisible) {
					var tempText = id.textVal.substring(0,id.startSelection) + id.textVal.substring(id.endSelection,id.textVal.length);
					id.textVal = tempText;
					id.cursorPosition = id.startSelection - 1;
					id.releaseShift();
				}
				else { 
					if (id.cursorPosition == id.textVal.length - 1) {
						//cursor is at the end of textVal
						id.textVal=id.textVal.substring(0,id.textVal.length-1);
					}
					else {
						//cursor is in between
						var tempText = id.textVal.substring(0,(id.cursorPosition)) + id.textVal.substring((id.cursorPosition + 1),(id.textVal.length));
						id.textVal = tempText;
					}
					//decrease cursor position
					if (id.cursorPosition > -1) {
						id.cursorPosition--;
					}
				}
				if(id.id=="code")
					id.textboxTextContent.nodeValue = id.textboxTextContent.nodeValue.substring(0,id.textboxTextContent.nodeValue.length-1);
				else
				    id.textboxTextContent.nodeValue =id.textVal;

				id.setCursorPos();
				if (id.cursorPosition > 0) {
					//retransform text element when cursor is at the left side of the box
					if (id.supportsCharGeom) {
						var cursorX = id.textboxText.getStartPositionOfChar(id.cursorPosition).x;
					}
					else {
						var bbox = id.textboxText.getBBox();
						var cursorX = bbox.x + bbox.width;
					}
					if ((cursorX + id.transX) < (id.x + id.textStyles["font-size"] / 3)) {
						id.transX += (id.x + id.textStyles["font-size"] / 3) - (cursorX + id.transX);
						if (id.transX * -1 < (id.boxWidth - id.textStyles["font-size"])) {
							id.transX = 0;
						}
						id.textboxTextGroup.setAttributeNS(null,"transform","translate("+id.transX+",0)");
					}
				}
				id.changed = true;
			}
...
textbox.prototype.specialCharacters = function(evt,DELflag) {
	//alert(evt.keyCode);
...
textbox.prototype.setCursorPos = function() {
		...
		if (this.cursorPosition > -1) {
			if (this.supportsCharGeom) {
				...
				this.textboxursor.setAttributeNS(null,"x1",cursorPos);
				this.textboxursor.setAttributeNS(null,"x2",cursorPos);
			}
			else {
				//case MozillaSVG 1.5 or other SVG viewers not implementing .getEndPositionOfChar
				var bbox = this.textboxText.getBBox();
				this.textboxursor.setAttributeNS(null,"x1",(bbox.x + bbox.width + this.textStyles["font-size"] / 3));
				this.textboxursor.setAttributeNS(null,"x2",(bbox.x + bbox.width + this.textStyles["font-size"] / 3));
			}
		}
		else {
...
textbox.prototype.fireFunction = function() {
	...
	if (typeof(this.functionToCall) == "function") {
		this.functionToCall(this,this.textVal,changeType);
	}
	if (typeof(this.functionToCall) == "object") {
		this.functionToCall.textboxhanged(this,this.textVal,changeType);	
	}
	if (typeof(this.functionToCall) == undefined) {
		return;
	}
//	alert(typeof(this.functionToCall)+changeType);
