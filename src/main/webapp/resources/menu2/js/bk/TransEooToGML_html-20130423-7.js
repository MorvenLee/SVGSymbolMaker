	...
	var defUploadFolder = "D:/work/tomcatWeb/uploadtest/";
 	...
 	{	
			var fileUpSrc = SvgMainDoc.getElementById("upload_fileSrc").firstChild.nodeValue;
			var attr_selVal = SvgMainDoc.getElementById("attr_selVal").firstChild.nodeValue;
			var splitAttr_selVal = SvgMainDoc.getElementById("splitAttr_selVal").firstChild.nodeValue;		
	...
	if(xmlHttp.readyState == 4)
	{
		if(xmlHttp.status == 200)
		{
      		...
      		//清空原信息
				...
				SvgMainDoc.getElementById("disDownInfo").firstChild.nodeValue = rt;
    		//alert(rt);
		}
		else{alert("数据转换失败!");}
	}
...
function setFileHiddenSrc(xmlHttpReq,index)
{
	if(xmlHttpReq.readyState == 4)
	{
		if(xmlHttpReq.status == 200)
		{
			...
			SvgMainDoc.getElementById("upload_fileSrc").firstChild.nodeValue = rt;		
			
			SvgMainDoc.svgWnd.getAttrdata(rt);
