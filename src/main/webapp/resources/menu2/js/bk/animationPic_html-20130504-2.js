	...
	var PicNum = 0;
 	...
 	function transSubmit(fileUpSrc,attr_selVal,splitAttr_selVal)
 	{			
			var sendStr = "filename=" + fileUpSrc.trim() + "&attr=" + attr_selVal + "&splitAttr=" + splitAttr_selVal;
			///*	
 			//---nancy edit 20130422---
			subXMLHTTP = createXMLHttpRequestCom();			

			subXMLHTTP.onreadystatechange = function() { disDownRes(subXMLHTTP);};
			subXMLHTTP.open("post","../../dataProcess/transformE00ToGML.jsp",true);
...
function disDownRes(xmlHttp)
{
	if(xmlHttp.readyState == 4)
	{
		if(xmlHttp.status == 200)
		{
      		...
      		//清空原信息
				var root = SvgRightDoc.rootElement;
				var SvgLev1 = SvgRightDoc.getElementById("SvgLev1");
				//alert(UpTopgObj);
				if(SvgLev1!=null)
				{					
				...
				SvgRightDoc.getElementById("SvgLev2").setAttributeNS(null,"visibility","visible");
				/*
				SvgRightDoc.getElementById("disDownInfo").firstChild.nodeValue = "下载";
				makeLink(SvgRightDoc.getElementById("disDownInfo"), rt);
				*/
				var tmp = rt.trim().split("|");
				//alert(tmp.length);
				var disInfoObj = SvgRightDoc.getElementById("disDownInfo");
				for(var i=0;i<tmp.length;i++)
				{
					var tspanText = SvgRightDoc.createElementNS(xmlns,"tspan");
				...
				/*
				var height = 500;
				SvgRightDoc.svgWnd.init_scrollbar(height);
				*/
			//初始化滚动条参数
			if(tmp.length>13)
			{
				...
				SvgRightDoc.svgWnd.init_scrollbar(barheight,diffHeight,percentH);//加载滚动条
			}
    		//alert(rt);
		}
		else{alert("数据转换失败!");}
