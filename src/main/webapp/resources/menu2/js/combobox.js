...
/////////////////////////////////////////////////////////////////////
//nancy add 20130504
combobox.prototype.getAllElements = function() {
	//alert("-----getAllElements-------");
	//alert(this.elementsArray.length);
	var elmStr = "";
	for (var i=0;i<this.elementsArray.length;i++) {
		//see how many and where elements are already selected
		var tmp =i;
		var tmpKey = this.elementsArray[i].key;
		var tmpKeyType = typeof(tmpKey);
		//var joinKey = tmpKey.join(";");
		//alert(joinKey);
		/*
		//if(this.elementsArray[i].key.trim()=="") continue;
		if(joinKey.trim()=="") continue;
		*/
		if(tmpKeyType== "object")
			elmStr += this.elementsArray[i].key + ";";
		else if(tmpKeyType== "string")
		{
			if(this.elementsArray[i].key.trim()=="") continue;
			elmStr += this.elementsArray[i].key + ";";
		}
