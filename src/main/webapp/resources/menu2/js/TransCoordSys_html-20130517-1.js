	...
	var destfileSrcArr = new Array() ;
 	...
 	function transSubmit(fileUpSrc,selectVal,select2Val,CPCoordVal_x1,CPCoordVal_y1,CPCoordVal_x2,CPCoordVal_y2)
 	{			
			var sendStr = "filename=" + fileUpSrc.trim() + "&selectVal=" + selectVal + "&select2Val=" + select2Val;
			sendStr += "&CPCoordVal_x1=" + CPCoordVal_x1 + "&CPCoordVal_y1=" + CPCoordVal_y1 ;
			sendStr += "&CPCoordVal_x2=" + CPCoordVal_x2 + "&CPCoordVal_y2=" + CPCoordVal_y2 ;			
			///*	
 			//---nancy edit 20130422---
			subXMLHTTP = createXMLHttpRequestCom();			

			subXMLHTTP.onreadystatechange = function() { disDownRes(subXMLHTTP);};
			subXMLHTTP.open("post","../../dataProcess/TransCoordSys.jsp",true);
 ...
 function isShp(filename){
       ...
       return true;
    	...
    	function handleDrop(evt){
    		...
    		var files = evt.dataTransfer.files;
				...
				if(!isShp(files[0].name))
				{
					alert("文件类型不正确，请上传shp文件!");
					return true;
				}
	...
	BarText.appendChild(tv);	
    	...
    	function uploadFile(file, progressbar) {
				...
				xhr.open("POST", "../../dataProcess/TransCoordSysUpload.jsp?fileName="+file.name);
