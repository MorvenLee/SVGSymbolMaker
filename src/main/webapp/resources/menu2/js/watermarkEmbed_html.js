	...
	var destfileTypeArr = new Array() ;
 	...
 	function transSubmit(method,svgfileName,userInfo_Val,sPass_Val,alias_Val,kPass_Val)
 	{
 		alert("开始导入数据库，请耐心等待。。。");			
			...
			subXMLHTTP.onreadystatechange = function() { disSuccessRes(subXMLHTTP,svgfileName,method);};
			...
			{
				subXMLHTTP.open("post","../../dataProcess/wmarkEmbed_featurenode.jsp",true);
			}
			else if(method=="promote")
			{
				subXMLHTTP.open("post","../../dataProcess/wmarkEmbed_promote.jsp",true);
			}
			else
			{
				subXMLHTTP.open("post","../../dataProcess/watermarkEmbed.jsp",true);
...
function disSuccessRes(xmlHttp,svgfileName,method)
{
	
		if(method=="lineSplit")  		
    		svgfileName = "w_" + svgfileName;
    	else if(method=="featurenode")
		{
    		svgfileName = "FPw_" + svgfileName;
		}
		else if(method=="promote")
		{
		...
		if(xmlHttp.status == 200)
		{
      		...
      		//清空原信息
				...
				if(rt.trim()=="success")
				{
					...
					makeLink(tspanText, "/SircNancy/integration/dataProcess/wdownloadWaterMark.jsp?filename=" + svgfileName + "&method=" +　method);
