...
var TextGapWidth = new Array(40,40,40,55,
							55,60,55,55
							);	//显示间距
...
function setFontSvg(index,textVal){
	...
	oprNumArray[flag1-1] = index;

	if(flag1>1)
	{
		preIndex = oprNumArray[flag1 - 2];
	}
	//alert("--oprNumArr" + parseInt(flag1-1) + "--flag1:" +　flag1 + "---index:" + index + "---pre:" + preIndex);
		
	if(index==5)
	{
		var selId = svgdoc_DW.getElementById("selPos").firstChild.nodeValue;
		id = directionArr[index-1];
		disOperVal = dirCNArr[selId];		
	...
	else if(index==6)
	{
		...
		if(flag1==1){
			...
			tid = "A" + flag1 ;
			disText = " " + disOperVal ;
			TextoffsetLeft = TextoffsetLeft + TextGapWidth[preIndex-1];
	...
	if(flag1==1){
			...
			oprNumArray[flag1-1] = index;
			//alert("--oprNumArray" + parseInt(flag1-1) + "--flag1:" +　flag1 + "   ---   index:" + index);
			//创建对象A2
			tid = "A" + flag1;		
			...
			disText = " " + disOperVal + " " + "A" + flag1;
			TextoffsetLeft = TextoffsetLeft + TextGapWidth[preIndex-1];
			//alert( "flag1:" + flag1 + "---preIndex:" + preIndex + "---offset:" +　TextoffsetLeft);
	...
	function appeTextChild(flag1,tid,posX,disText)
	{
		...
		texts.setAttributeNS(null,"cursor","pointer");		
		texts.setAttributeNS(null,"onclick", "window.parent.markOp(" + flag1 + ");window.open('../page/selectSVG.html','','width=500px,height=300px,status=yes');");
...
function addLayer(layer){
	var svgdoc = window.document.getElementById("svgfile1").getSVGDocument();
	...
	//alert(layer);
	var obj = parseSVGXML("<g id=\"layer"+layerID+"\" fill=\"none\" stroke=\"black\">"+layer+"</g>", svgdoc);
	...
	svgdoc.svgWnd.init_dyckBox(layerID);
...
//------nancy edit 0330-----
function onSelect(index,checkVal){
	var svgdoc = document.getElementById("svgfile1").getSVGDocument();
	var tmp = svgdoc.getElementById(index);
	
		if (checkVal == true){
			svgdoc.getElementById(index).style.setProperty("visibility", "visible");
		}else
			svgdoc.getElementById(index).style.setProperty("visibility", "hidden");
}
...
function OnCal(){
	...
	var childs2 = lay_result2.childNodes;
		if (childs2.length != 0){
	...
	for (var i=0; i<ret2.length; i++){
		...
		var path = svgdoc2.createElementNS(xmlns,"path");
